...
test.describe("ワークスペース管理フロー", () => {
  test("ワークスペースを作成できる", async ({ page }) => {
    ...
    // モーダルが表示される
    await expect(
      page.getByRole("heading", { name: "ワークスペースを作成" })
    ).toBeVisible();
  ...
  test("ワークスペース設定画面が表示される", async ({ page }) => {
    ...
    // 招待リンクセクションが表示される
    await expect(
      page.getByRole("heading", { name: "招待リンク" })
    ).toBeVisible();

    // 危険な操作セクションが表示される
    await expect(
      page.getByRole("heading", { name: "危険な操作" })
    ).toBeVisible();
  });
  ...
  test("招待リンクを発行できる", async ({ page }) => {
    ...
    // リンクが表示される
    await expect(
      page.locator('input[value*="https://wavey.app/invite"]')
    ).toBeVisible();
  ...
  test("オーナー権限譲渡モーダルが表示される", async ({ page }) => {
    ...
    await expect(
      page.getByText("この操作は取り消せません", { exact: true })
    ).toBeVisible();
  ...
  test("ワークスペース削除モーダルが表示される", async ({ page }) => {
    ...
    // 確認モーダルが表示される
    await expect(
      page.getByText("すべてのデータが削除されます")
    ).toBeVisible();

    // ワークスペース名入力フィールドが表示される
    await expect(
      page.getByText("確認のためワークスペース名を入力")
    ).toBeVisible();
